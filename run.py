...
วิธีใช้:
    ...
    python run.py agent      # รัน Orchestrator.run_task() จริงบน saucedemo.com
                              ...
                              # step ลง terminal คู่กัน (ต้องมี ANTHROPIC_API_KEY
                              # จริงใน .env)
"""
...
_DEFAULT_AGENT_GOAL = "Log in as standard_user/secret_sauce แล้วเพิ่ม SauceLabsBackpack ลงตะกร้า"
...
def run_agent():
    ...
    async def _run():
        result = await Orchestrator().run_task(
            "https://www.saucedemo.com/", goal, headless=False, verbose=True, provider=provider
        )
        print("\n=== ผลลัพธ์ ===")
        ...
        print(f"tokens : input={tokens['input']} output={tokens['output']} total={tokens['input'] + tokens['output']}")
        print("history:")

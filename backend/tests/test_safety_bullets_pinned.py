...
# 6 frozen bullet(s) that must live in _PROMPT_CORE
_FROZEN_CORE = [
...
# 5 frozen bullet(s) that must live in _PROMPT_TABLE
_FROZEN_TABLE = [
...
# 1 frozen bullet(s) that must live in _PROMPT_WIDGET
_FROZEN_WIDGET = [
...
def test_safety_bullets_are_pinned_verbatim():
    """Every frozen bullet must appear, character for character, in the assembled
    system prompt (and in the block it belongs to). Fails on any wording change,
    internal reorder, partial rewrite, or deletion."""
    core = llm.build_system_prompt(frozenset())
    table = llm.build_system_prompt(frozenset({"table"}))
    widget = llm.build_system_prompt(frozenset({"widget"}))
    full = llm.build_system_prompt()

    for bullet in _FROZEN_CORE:
        assert bullet in core, "W21/W63/W64 core bullet changed or gone: " + bullet[:110]
        assert bullet in full
    for bullet in _FROZEN_TABLE:
        assert bullet in table, "W21/W63/W64 table bullet changed or gone: " + bullet[:110]
        assert bullet in full
    for bullet in _FROZEN_WIDGET:
        assert bullet in widget, "W50 widget bullet changed or gone: " + bullet[:110]
        assert bullet in full

    # Count guard: exactly this many protected bullets, no more, no fewer. A newly
    # added W21/W50/W63/W64 bullet must be pinned here too; a removed one must be a
    # deliberate, reviewed change.
    assert len(_protected_bullets(llm._PROMPT_CORE)) == len(_FROZEN_CORE)
    assert len(_protected_bullets(llm._PROMPT_TABLE)) == len(_FROZEN_TABLE)
    assert len(_protected_bullets(llm._PROMPT_WIDGET)) == len(_FROZEN_WIDGET)
...
def test_safety_bullets_match_the_source_blocks_exactly():
    ...
    assert _protected_bullets(llm._PROMPT_CORE) == _FROZEN_CORE
    assert _protected_bullets(llm._PROMPT_TABLE) == _FROZEN_TABLE
    assert _protected_bullets(llm._PROMPT_WIDGET) == _FROZEN_WIDGET
